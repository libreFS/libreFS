# libreFS Pull Request Guidelines

These guidelines ensure high-quality commits in libreFS's GitHub repositories, maintaining
a clear, valuable commit history for our open-source project. They apply to all contributors,
fostering efficient reviews and robust code.

## Why Pull Requests?

Pull Requests (PRs) drive quality in libreFS's codebase by:
...
- Adheres to libreFS's coding standards (Go style, error handling, testing).
...
1. **Title and Summary**:
   ...
   - Use GitHub draft PRs for work in progress to avoid premature merging.
...
2. **Commits**:
   ...
   - Rebase to tidy commits before submitting (`git rebase -i master` to squash typos or reword messages), unless multiple contributors worked on the branch.
...
3. **Testing**:
   ...
   - Ensure existing tests pass (`make -f Makefile.docker test`).
   - Document testing steps in the PR summary if manual testing was performed.

4. **Before Submitting**:
   - Run `make -f Makefile.docker verifiers` to check formatting, linting, and tests.
   - Reference related issues (e.g., "Closes #1234").
   - Notify contributors via GitHub `@mentions` if the change is urgent or complex.

## Reviewing PRs

Reviewers ensure libreFS's commit history remains a clear, reliable record. Responsibilities include:
...
2. **Code Quality**:
   - Check adherence to Go standards (error handling, documentation, style).
...
3. **Flow**:
   ...
   - If unable to complete a review, tag another contributor (e.g., `@username please take over`).

4. **Shared Responsibility**:
   - All libreFS contributors are reviewers. The first commenter on a PR owns the review unless they delegate.
...
5. **No Self-Edits**:
   ...
   - If you edit, you are a collaborator, not a reviewer, and cannot merge.
...
- **Engage Early**: Discuss complex changes in [GitHub Issues](https://github.com/libreFS/libreFS/issues) before coding.
...
- [libreFS Contributing Guide](CONTRIBUTING.md)
...
By following these guidelines, we ensure libreFS's codebase remains high-quality, maintainable, and a joy to contribute to. Happy coding!
