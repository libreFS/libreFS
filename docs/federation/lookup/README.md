...
This document explains how to configure libreFS with `Bucket lookup from DNS` style federation.
...
### 2. Run libreFS in federated mode
...
This is comma separated list of etcd servers that you want to use as the libreFS federation back-end. This should
be same across the federated deployment, i.e. all the libreFS instances within a federated deployment should use same
etcd back-end.
...
running in front of all the federated libreFS instances. The domain name is used to create sub domain entries to etcd. For
...
This is comma separated list of IP addresses to which buckets created on this libreFS instance will resolve to. For example,
a bucket `bucket1` created on current libreFS instance will be accessible as `bucket1.domain.com`, and the DNS entry for
`bucket1.domain.com` will point to IP address set in `MINIO_PUBLIC_IPS`.

- This field is mandatory for standalone and erasure code libreFS server deployments, to enable federated mode.
...
hosts passed to the libreFS server startup and use them for DNS entries.
...
librefs server http://rack{1...4}.host{1...4}.domain.com/mnt/export{1...32}
```
...
librefs server http://rack{5...8}.host{5...8}.domain.com/mnt/export{1...32}
```

In this configuration you can see `MINIO_ETCD_ENDPOINTS` points to the etcd backend which manages libreFS's
...
To test this setup, access the libreFS server via browser or [`mc`](https://docs.min.io/community/minio-object-store/reference/minio-mc.html#quickstart). You’ll see the uploaded files are accessible from the all the libreFS endpoints.
