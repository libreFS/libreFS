# libreFS FTP/SFTP Server

libreFS natively supports FTP/SFTP protocol, this allows any ftp/sftp client to upload and download files.
...
libreFS supports following FTP/SFTP based protocols to access and manage data.
...
Start libreFS in a distributed setup, with 'ftp/sftp' enabled.

```
librefs server http://server{1...4}/disk{1...4}
...
220 Welcome to libreFS FTP Server
...
> NOTE: if libreFS distributed setup is already configured to run under TLS, FTP will automatically use the relevant
