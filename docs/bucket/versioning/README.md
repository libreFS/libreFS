...
libreFS versioning is designed to keep multiple versions of an object in one bucket. For example, you could store `spark.csv` (version `ede336f2`) and `spark.csv` (version `fae684da`) in a single bucket. Versioning protects you from unintended overwrites, deletions, protect objects with retention policies.
...
When you DELETE an object, all versions remain in the bucket and libreFS adds a delete marker, as shown below:
...
- All Buckets on libreFS are always in one of the following states: unversioned (the default) and all other existing deployments, versioning-enabled, or versioning-suspended.
...
To enable versioning, you send a request to libreFS with a versioning configuration with Status set to `Enabled`.
...
## libreFS extension to Bucket Versioning
...
## Examples of enabling bucket versioning using libreFS Java SDK
...
- [Use `minio-java` SDK with libreFS Server](https://docs.min.io/community/minio-object-store/developers/java/minio-java.html)
