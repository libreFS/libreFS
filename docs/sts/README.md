# libreFS STS Quickstart Guide [![Slack](https://slack.min.io/slack?type=svg)](https://slack.min.io)

The libreFS Security Token Service (STS) is an endpoint service that enables clients to request temporary credentials for libreFS resources. Temporary credentials work almost identically to default admin credentials, with some differences:

- Temporary credentials are short-term, as the name implies. They can be configured to last for anywhere from a few minutes to several hours. After the credentials expire, libreFS no longer recognizes them or allows any kind of access from API requests made with them.
...
| [**AssumeRole**](https://github.com/minio/minio/blob/master/docs/sts/assume-role.md)   | Let libreFS users request temporary credentials using user access and secret keys.                                                              |
...
### Setup libreFS with Identity Provider

Make sure we have followed the previous step and configured each software independently, once done we can now proceed to use libreFS STS API and libreFS server to use these credentials to perform object API operations.
