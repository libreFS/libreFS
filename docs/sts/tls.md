...
libreFS provides a custom STS API that allows authentication with client X.509 / TLS certificates.
...
The libreFS TLS STS API can be configured via libreFS's standard configuration API (i.e. using `mc admin config set/get`). Further, it can be configured via the following environment variables:
...
The libreFS TLS STS API is disabled by default. However, it can be *enabled* by setting environment variable:
...
libreFS exposes a custom S3 STS API endpoint as `Action=AssumeRoleWithCertificate`. A client has to send an HTTP `POST` request to `https://<host>:<port>?Action=AssumeRoleWithCertificate&Version=2011-06-15`. Since the authentication and authorization happens via X.509 certificates the client has to send the request over **TLS** and has to provide
a client certificate.

The following curl example shows how to authenticate to a libreFS server with client certificate and obtain STS access credentials.
...
A client can request temp. S3 credentials via the STS API. It can authenticate via a client certificate and obtain a access/secret key pair as well as a session token. These credentials are associated to an S3 policy at the libreFS server.

In case of certificate-based authentication, libreFS has to map the client-provided certificate to an S3 policy. libreFS does this via the subject common name field of the X.509 certificate. So, libreFS will associate a certificate with a subject `CN = foobar` to a S3 policy named `foobar`.

The following self-signed certificate is issued for `consoleAdmin`. So, libreFS would associate it with the pre-defined `consoleAdmin` policy.
...
Also, note that the certificate has to contain the `Extended Key Usage: TLS Web Client Authentication`. Otherwise, libreFS would not accept the certificate as client certificate.

Now, the STS certificate-based authentication happens in 4 steps:

- Client sends HTTP `POST` request over a TLS connection hitting the libreFS TLS STS API.
- libreFS verifies that the client certificate is valid.
- libreFS tries to find a policy that matches the `CN` of the client certificate.
- libreFS returns temp. S3 credentials associated to the found policy.
...
Further, the temp. S3 credentials will never out-live the client certificate. For example, if the `MINIO_IDENTITY_TLS_STS_EXPIRY` is 7 days but the certificate itself is only valid for the next 3 days, then libreFS will return S3 credentials that are valid for 3 days only.
