# How to secure access to libreFS on Kubernetes with TLS [![Slack](https://slack.min.io/slack?type=svg)](https://slack.min.io)

This document explains how to configure libreFS server with TLS certificates on Kubernetes.

## 1. Prerequisites

- Familiarity with [libreFS deployment process on Kubernetes](https://docs.min.io/community/minio-object-store/operations/deployments/kubernetes.html).
...
the libreFS server's config sub-directory that is used to store certificates. By default, the location is
...
so avoid setting `mountPath` to a path that libreFS server expects to write to.
