...
libreFS supports encrypting config, IAM assets with KMS provided keys. If the KMS is not enabled, libreFS will store the config, IAM data as plain text erasure coded in its backend.

## libreFS KMS Quick Start

libreFS supports two ways of encrypting IAM and configuration data.
You can either use KES - together with an external KMS - or, much simpler,
set the env. variable `MINIO_KMS_SECRET_KEY` and start/restart the libreFS server. For more details about KES and how
...
For distributed libreFS deployments, specify the *same* `MINIO_KMS_SECRET_KEY` for each libreFS server process.
...
- For instructions on using KES for encrypting the libreFS backend, follow the [KMS Quick Start](https://github.com/minio/minio/tree/master/docs/kms). The SSE-S3 configuration setup also supports libreFS KMS backend encryption.
...
Now, libreFS encrypts IAM / configuration and S3 objects with a KMS, if present. This
change unified the key-management aspect within libreFS.
...
- Reduced server startup time. For IAM encryption with the root credentials, libreFS had
...
> Does this mean I need an enterprise KMS setup to run libreFS (securely)?

No, libreFS does not depend on any third-party KMS provider. You have three options here:

- Run libreFS without a KMS. In this case all IAM data will be stored in plain-text.
- Run libreFS with a single secret key. libreFS supports a static cryptographic key
...
- Run libreFS with KES (minio/kes) in combination with any supported KMS as
  secure key store. For example, you can run libreFS + KES + Hashicorp Vault.

> What about an exiting libreFS deployment? Can I just upgrade my cluster?

Yes, libreFS will try to transparently migrate any existing IAM data and either stores
...
configuration itself can no longer be stored in the libreFS config file and
...
> Will an upgrade of an existing libreFS cluster impact the SLA of the cluster or will it even cause downtime?

No, an upgrade should not cause any downtime. However, on the first startup -
since libreFS will attempt to migrate any existing IAM data - the boot process may
