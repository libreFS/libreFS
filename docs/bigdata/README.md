# **Disaggregated HDP Spark and Hive with libreFS**
...
All access to libreFS object storage is via S3/SQL SELECT API. In addition to the compute nodes, libreFS containers are also managed by Kubernetes as stateful containers with local storage (JBOD/JBOF) mapped as persistent local volumes. This architecture enables multi-tenant libreFS, allowing isolation of data between customers.

libreFS also supports multi-cluster, multi-site federation similar to AWS regions and tiers. Using libreFS Information Lifecycle Management (ILM), you can configure data to be tiered between NVMe based hot storage, and HDD based warm storage. All data is encrypted with per-object key. Access Control and Identity Management between the tenants are managed by libreFS using OpenID Connect or Kerberos/LDAP/AD.
...
- Install libreFS Distributed Server using one of the guides below.
  - [Deployment based on Kubernetes](https://docs.min.io/community/minio-object-store/operations/deployments/kubernetes.html)
  - [Deployment based on libreFS Helm Chart](https://github.com/helm/charts/tree/master/stable/minio)

## **3. Configure Hadoop, Spark, Hive to use libreFS**
...
Navigate to **Custom core-site** to configure libreFS parameters for `_s3a_` connector
...
Let's take for example a set of 12 compute nodes with an aggregate memory of _1.2TiB_, we need to do following settings for optimal results. Add the following optimal entries for _core-site.xml_ to configure _s3a_ with **libreFS**. Most important options here are
...
fs.s3a.committer.threads=2048 # Number of threads writing to libreFS
...
Navigate to “**Custom spark-defaults**” to configure libreFS parameters for `_s3a_` connector

![spark-config](https://github.com/minio/minio/blob/master/docs/bigdata/images/image9.png?raw=true "spark defaults")

Add the following optimal entries for _spark-defaults.conf_ to configure Spark with **libreFS**.
...
spark.hadoop.fs.s3a.committer.threads 2048 # number of threads writing to libreFS
...
Navigate to “**Custom hive-site**” to configure libreFS parameters for `_s3a_` connector

![hive-config](https://github.com/minio/minio/blob/master/docs/bigdata/images/image11.png?raw=true "hive advanced config")

Add the following optimal entries for `hive-site.xml` to configure Hive with **libreFS**.
...
- Login as user **‘spark’**.
- When the job runs, the library can now use **libreFS** during intermediate processing.
...
To view the output from libreFS exit the Scala shell. View WordCount job status:
