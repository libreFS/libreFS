# libreFS Logging Quickstart Guide [![Slack](https://slack.min.io/slack?type=svg)](https://slack.min.io)

This document explains how to configure libreFS server to log to different logging targets.

## Log Targets

libreFS supports currently two target types
...
HTTP target logs to a generic HTTP endpoint in JSON format and is not enabled by default. To enable HTTP target logging you would have to update your libreFS server configuration using `mc admin config set` command.
...
libreFS also honors environment variable for HTTP target logging as shown below, this setting will override the endpoint settings in the libreFS server config.
...
libreFS also honors environment variable for HTTP target Audit logging as shown below, this setting will override the endpoint settings in the libreFS server config.
...
```json
{
  ...
  "userAgent": "libreFS (linux; amd64) minio-go/v7.0.70 mc/RELEASE.2024-04-30T17-44-48Z",
  ...
  "requestHeader": {
    ...
    "User-Agent": "libreFS (linux; amd64) minio-go/v7.0.70 mc/RELEASE.2024-04-30T17-44-48Z",
...
Configure libreFS to send audit logs to locally running Kafka brokers
