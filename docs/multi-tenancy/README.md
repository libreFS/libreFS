# libreFS Multi-Tenant Deployment Guide [![Slack](https://slack.min.io/slack?type=svg)](https://slack.min.io) [![Docker Pulls](https://img.shields.io/docker/pulls/minio/minio.svg?maxAge=604800)](https://hub.docker.com/r/minio/minio/)
...
To host multiple tenants on a single machine, run one libreFS Server per tenant with a dedicated HTTPS port, configuration, and data directory.
...
```sh
librefs server --address :9001 /data/tenant1
librefs server --address :9002 /data/tenant2
librefs server --address :9003 /data/tenant3
```
...
```sh
librefs server --address :9001 /disk{1...4}/data/tenant1
librefs server --address :9002 /disk{1...4}/data/tenant2
librefs server --address :9003 /disk{1...4}/data/tenant3
```
...
To host multiple tenants in a distributed environment, run several distributed libreFS Server instances concurrently.
...
librefs server --address :9001 http://192.168.10.1{1...4}/data/tenant1

export MINIO_ROOT_USER=<TENANT2_ACCESS_KEY>
export MINIO_ROOT_PASSWORD=<TENANT2_SECRET_KEY>
librefs server --address :9002 http://192.168.10.1{1...4}/data/tenant2

export MINIO_ROOT_USER=<TENANT3_ACCESS_KEY>
export MINIO_ROOT_PASSWORD=<TENANT3_SECRET_KEY>
librefs server --address :9003 http://192.168.10.1{1...4}/data/tenant3
```
...
**Note**: On distributed systems, root credentials are recommend to be defined by exporting the `MINIO_ROOT_USER` and  `MINIO_ROOT_PASSWORD` environment variables. If no value is set libreFS setup will assume `minioadmin/minioadmin` as default credentials. If a domain is required, it must be specified by defining and exporting the `MINIO_DOMAIN` environment variable.

## Cloud Scale Deployment

A container orchestration platform (e.g. Kubernetes) is recommended for large-scale, multi-tenant libreFS deployments. See the [libreFS Deployment Quickstart Guide](https://docs.min.io/community/minio-object-store/operations/deployments/kubernetes.html) to get started with libreFS on orchestration platforms.
