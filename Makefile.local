...
verify: install-race ## verify librefs various setups
	@echo "Verifying build with race"
	@(env bash $(PWD)/buildscripts/verify-build.sh)

verify-healing: install-race ## verify healing and replacing disks with librefs binary
...
build: checks build-debugging ## builds librefs to $(PWD)
	@echo "Building librefs binary to './librefs'"
	@CGO_ENABLED=0 GOOS=$(GOOS) GOARCH=$(GOARCH) go build -tags kqueue -trimpath --ldflags "$(LDFLAGS)" -o $(PWD)/librefs 1>/dev/null
...
install-race: checks build-debugging ## builds librefs to $(PWD)
	@echo "Building librefs binary with -race to './librefs'"
	@GORACE=history_size=7 CGO_ENABLED=1 go build -tags kqueue,dev -race -trimpath --ldflags "$(LDFLAGS)" -o $(PWD)/librefs 1>/dev/null
	@echo "Installing librefs binary with -race to '$(GOPATH)/bin/librefs'"
	@mkdir -p $(GOPATH)/bin && cp -af $(PWD)/librefs $(GOPATH)/bin/librefs

install: build ## builds librefs and installs it to $GOPATH/bin.
	@echo "Installing librefs binary to '$(GOPATH)/bin/librefs'"
	@mkdir -p $(GOPATH)/bin && cp -af $(PWD)/librefs $(GOPATH)/bin/librefs
	@echo "Installation successful. To learn more, try \"librefs --help\"."
